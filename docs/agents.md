...
This document provides quick curl examples and expected responses for the `agents` endpoints, including `ETag`/`If-Match` usage. Keep these snippets handy when developing the frontend (see `client/README.md` for dev workflow).
...
- `PUT /agents/{id}` → 204 NoContent or 400/404/412
...
```bash
# fetch agent and inspect headers (look for ETag)
